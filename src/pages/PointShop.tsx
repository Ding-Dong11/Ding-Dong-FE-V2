...
function SearchBar() {
  return (
    <label className="mx-5 flex h-[52px] items-center gap-2.5 rounded-full border border-neutral-200 px-5 py-3.5">
...
function ExchangeModal({ onClose, onExchange }: { onClose: () => void; onExchange: () => void }) {
  const [done, setDone] = useState(false);
  return (
    <div className="absolute inset-0 z-30 flex flex-col justify-start bg-black/20 px-5 pt-40" onClick={onClose}>
      <div className="pop-in rounded-3xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
  ...
  return (
    <div className="relative flex flex-1 flex-col">
      <div className="flex-1 overflow-y-auto pb-6">
        <h1 className="py-6 text-center text-xl font-extrabold">포인트 사용</h1>
        <SearchBar />
        <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-8 px-5">
          {items.map((item) => (
            <button key={item.id} type="button" disabled={item.exchanged} onClick={() => setOpenId(item.id)} className="text-left">
              ...
              <p className={`mt-2.5 text-sm ${item.exchanged ? "text-neutral-300" : "text-sub"}`}>{PRODUCT.store}</p>
              <p className={`text-lg font-medium ${item.exchanged ? "text-neutral-300" : ""}`}>{PRODUCT.name}</p>
              <p className={`text-2xl font-extrabold ${item.exchanged ? "text-neutral-300" : ""}`}>{PRODUCT.price} P</p>
            </button>
          ))}
        </div>
      ...
      {openId !== null && (
        <ExchangeModal
          ...
          onExchange={() => setItems((prev) => prev.map((it) => (it.id === openId ? { ...it, exchanged: true } : it)))}
        />
      )}
      <BottomNav />
    </div>
